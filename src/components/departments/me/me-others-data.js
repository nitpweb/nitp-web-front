...

 {
  ...
  content: (
   <ul>
    ...
    <li>
     Team AAVEGA stood 19th in Supra SAEINDIA 2015 and 2nd among all the NITs
     and IITs.
    </li>
       ...
       <td>
        {Object.keys(row["Name of the Faculty"]).map((title, idx) => (
         <React.Fragment key={idx}>
          <strong>{title}:</strong> {row["Name of the Faculty"][title]}
          <br />
         </React.Fragment>
        ))}
       </td>
       <td>{row["Organized Workshop/FDP/STC/Conference/GIAN program"]}</td>
       <td>{row.Duration}</td>
      </tr>
     ))}
    </tbody>
