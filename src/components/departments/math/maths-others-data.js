export const Activities = {
  content: (
    <>
      <table className="table-act">
        <tr>
          <h2>Seminars and Workshops Organized by the Department</h2>

          <td>
            <b>Inspire Science Camp for School Children (15-19 Dec; 2014) </b>
          </td>
          <td>
            Short term course on
            <b>
              Time Series Analysis & its Applications jointly organized with
              Computer Science & Civil Engineering Department during June 2014.
            </b>
          </td>
          <td>
            Nationwide sensitization workshop on
            <b>Mathematics for Teachers and Activists</b> during 2-3 March; 2014
          </td>
          <td>
            Nationwide sensitization workshop on
            <b>Mathematics for Teachers and Activists</b> during 2-3 March;
            2014)
          </td>
        </tr>
      </table>
      <h2>Externally Funded Projects Running in the Department</h2>
      <table className="table-act">
        <tr>
          <td>
            <b>Sl. No</b>
          </td>
          <td>
            <b>Name of the projects</b>
          </td>
          <td>
            <b>Name of the faculty</b>
          </td>
          <td>
            <b>Major funding agency</b>
          </td>
          <td>
            <b>Completed / Ongoing</b>
          </td>
          <td>
            <b>Grant Sanctioned (INR in Lakhs)</b>
          </td>
        </tr>
        <tr>
          <td>1</td>
          <td>
            Next Generation E-content of Numerical Methods and Applications
          </td>
          <td>Prof. S. K. Singh</td>
          <td>MHRD</td>
          <td>Completed</td>
          <td>5</td>
        </tr>
        <tr>
          <td>2</td>
          <td>
            Numerical Analysis for Singularly Perturbed Parabolic Partial
            Differential Equations
          </td>
          <td>Dr. S. Gowrisankar</td>
          <td>SERB-DST</td>
          <td>Approved</td>
          <td>17</td>
        </tr>
      </table>
      <div className="row ">
        <h2>Invited Lectures</h2>
      </div>
      <div>
        {" "}
        <table className="table-act">
          <tr>
            <td>
              <b>Sl. No.</b>
            </td>
            <td>
              <b>Faculty</b>
            </td>
            <td>
              <b>Institute</b>
            </td>
          </tr>

          <tr>
            <td>1</td>
            <td>Prof. S. Chakrabarty , Maths Department </td>
            <td>NIT Rourkela</td>
          </tr>
          <tr>
            <td>2</td>
            <td>Prof. A. K. Ganguly , Maths Department </td>
            <td>Patna University</td>
          </tr>
          <tr>
            <td>3</td>
            <td>Prof. K. N. Rai , Maths Department </td>
            <td>IIT BHU</td>
          </tr>
          <tr>
            <td>4</td>
            <td>Prof. S. Natesan , Maths Department </td>
            <td>IIT Guwahati </td>
          </tr>
          <tr>
            <td>5</td>
            <td>Prof. K. N. Rai , Maths Department </td>
            <td>IIT BHU</td>
          </tr>
          <tr>
            <td>6</td>
            <td>Prof. S. Bhatacharyya , Maths Department </td>
            <td>IIT Kharagpur </td>
          </tr>
          <tr>
            <td>7</td>
            <td>Prof. Santhana , Maths Department </td>
            <td>IIT Kanpur</td>
          </tr>
          <tr>
            <td>8</td>
            <td>Prof. Karmeshu , Maths & Computer Science Department </td>
            <td>J. N. U Delhi</td>
          </tr>
        </table>
      </div>
    </>
  ),
}
